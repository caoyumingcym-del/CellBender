...
class CLI(AbstractCLI):
    ...
    def validate_args(args) -> argparse.Namespace:
        ...
        if args.expected_cell_count is not None:
            assert args.expected_cell_count > 0, "expected_cells must be an integer greater than zero."

        # If additional barcodes are specified, they must be specified for
        # all files.
        if (args.total_droplets is not None) and (args.expected_cell_count is not None):
        ...
        for fpr in args.fpr:
            try:
                fpr = float(fpr)
                assert (fpr >= 0.0) and (fpr < 1.0), "False positive rate --fpr must be in [0, 1)"
            except ValueError:
                # the input is not a float
                assert fpr == "cohort", "The only allowed non-float value for FPR is the word 'cohort'."
        ...
        for feature in args.exclude_features:
            if feature not in allowed_features:
                sys.stdout.write(
                    f"Specified '{feature}' using --exclude-feature-types, "
        ...
        if "Gene Expression" in args.exclude_features:
            sys.stdout.write(
                "WARNING: Excluding 'Gene Expression' features from the analysis "
        ...
        if args.epoch_elbo_fail_fraction is not None:
            assert args.epoch_elbo_fail_fraction > 0.0, "--epoch-elbo-fail-fraction must be in > 0"
        if args.final_elbo_fail_fraction is not None:
            assert args.final_elbo_fail_fraction > 0.0, "--final-elbo-fail-fraction must be in > 0"
        ...
        if args.checkpoint_min > 15:
            sys.stdout.write(
                f"Warning: Timing between checkpoints is specified as "
        ...
        if args.estimator == "cdf":
            ...
            assert args.cdf_threshold_q is not None, "Input argument --q must be specified when using --estimator cdf"
