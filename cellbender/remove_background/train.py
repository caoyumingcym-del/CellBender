...
def is_scheduler(optim):
    ...
    if hasattr(optim, "is_scheduler"):
        ...
        return isinstance(optim, pyro.optim.lr_scheduler.PyroLRScheduler)
        return isinstance(optim, pyro.optim.lr_scheduler.PyroLRScheduler)


def train_epoch(svi: SVI, train_loader: DataLoader) -> float:
def train_epoch(svi: SVI, train_loader: DataLoader) -> float:
    ...
    if is_scheduler(svi.optim):
        try:
            logger.debug(f"Learning rate scheduler: LR = {list(svi.optim.optim_objs.values())[0].get_last_lr()[0]:.2e}")
            logger.debug(f"Learning rate scheduler: LR = {list(svi.optim.optim_objs.values())[0].get_last_lr()[0]:.2e}")
        except IndexError:
...
def evaluate_epoch(svi: pyro.infer.SVI, test_loader: DataLoader) -> float:
def evaluate_epoch(svi: pyro.infer.SVI, test_loader: DataLoader) -> float:
...
def run_training(
    ...
    try:
        start_epoch = (
            1 if (model is None) or (len(model.loss["train"]["epoch"]) == 0) else model.loss["train"]["epoch"][-1] + 1
        )
        start_epoch = (
            1 if (model is None) or (len(model.loss["train"]["epoch"]) == 0) else model.loss["train"]["epoch"][-1] + 1
        )
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            if epoch == start_epoch + 1:
                ...
                logger.info(
                    "[epoch %03d]  average training loss: %.4f  (%.1f seconds per epoch)"
                ...
                if (epoch_checkpoint_freq > 0) and (epoch_checkpoint_freq < epochs):
                    ...
                    logger.info(f"Will not checkpoint due to projected run completion in under {checkpoint_freq} min")
                    logger.info(f"Will not checkpoint due to projected run completion in under {checkpoint_freq} min")
            else:
                logger.info("[epoch %03d]  average training loss: %.4f" % (epoch, total_epoch_loss_train))
                logger.info("[epoch %03d]  average training loss: %.4f" % (epoch, total_epoch_loss_train))
            ...
            if (test_loader is not None) and (len(test_loader) > 0):
                ...
                if epoch % test_freq == 0:
                    ...
                    logger.info("[epoch %03d] average test loss: %.4f" % (epoch, total_epoch_loss_test))
                    model.loss["test"]["epoch"].append(epoch)
                    model.loss["test"]["elbo"].append(-total_epoch_loss_test)
                    logger.info("[epoch %03d] average test loss: %.4f" % (epoch, total_epoch_loss_test))
                    ...
                    if (epoch_elbo_fail_fraction is not None) and (len(model.loss["test"]["elbo"]) > 2):
                        current_diff = max(0.0, model.loss["test"]["elbo"][-2] - model.loss["test"]["elbo"][-1])
        ...
        if final_elbo_fail_fraction is not None:
            ...
            if model.loss["test"]["elbo"][-1] < best_test_elbo:
                ...
                if initial_best_diff == 0:
                    raise ElboException(
                        f"Training failed because there was no improvement from the initial test loss "
                        f"{model.loss['test']['elbo'][0]:.2f}. "
                        f"Training failed because there was no improvement from the initial test loss "
                        f"{model.loss['test']['elbo'][0]:.2f}. "
    ...
    # Exception allows program to continue after ending inference prematurely.
        ...
        logger.info("Inference procedure stopped by keyboard interrupt... will save a checkpoint.")
        save_checkpoint(
            filebase=output_filename,
            tarball_name=ckpt_tarball_name,
            args=args,
    ...
    # Exception allows program to produce output when terminated by a NaN.
        ...
        print(nan.message)
        logger.info(
            f"Inference procedure terminated early due to a NaN value in: {nan.param}\n\n"
    ...
    if (final_elbo_fail_fraction is not None) and (len(model.loss["test"]["elbo"]) > 1):
        ...
        if -model.loss["test"]["elbo"][-1] >= -best_test_elbo * (1 + final_elbo_fail_fraction):
            raise ElboException(
                f"Training failed because final test loss ({-model.loss['test']['elbo'][-1]:.4f}) "
